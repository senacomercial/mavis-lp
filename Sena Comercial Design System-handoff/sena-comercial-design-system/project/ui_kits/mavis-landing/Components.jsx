...
function UrgencyBadge({ lines }) {
  return (
    <div style={{
      background:'#CC0000', color:'#fff',
      fontFamily:'Montserrat,sans-serif', fontSize:15, fontWeight:800,
      lineHeight:1.4, padding:'14px 26px', textAlign:'center',
      letterSpacing:'.02em', width:'100%', maxWidth:340, borderRadius:2
    }}>
...
function MetricCard({ over, num, sub, variant = 'orange' }) {
...
function PricingCard({ over, name, price, pricePromo, includes, featured }) {
  return (
    <div style={{
      ...
      {featured && <div style={{position:'absolute',top:14,right:14,fontFamily:'Montserrat,sans-serif',fontSize:9,fontWeight:800,letterSpacing:'.18em',color:'#000',background:'#C5A059',padding:'4px 10px',borderRadius:2}}>PÓDIO</div>}
      ...
      <div style={{fontFamily:'Montserrat,sans-serif',fontSize:11,color:'rgba(255,255,255,.45)',letterSpacing:'.08em',textTransform:'uppercase',marginBottom:20}}>{pricePromo ? 'fechamento em call' : 'preço cheio'}</div>
      <ul style={{listStyle:'none',padding:0,margin:'0 0 24px',display:'flex',flexDirection:'column',gap:10}}>
        {includes.map((i,idx)=>(
          <li key={idx} style={{display:'flex',gap:10,fontFamily:'Montserrat,sans-serif',fontSize:13,color:'#eee',lineHeight:1.5}}>
            <span style={{color:'#C5A059',fontWeight:900}}>✓</span>{i}
          </li>
        ))}
      </ul>
      <CTAButton variant={featured?'primary':'ghost'}>Conversar com a Sena</CTAButton>
    </div>
  );
}

Object.assign(window, { ProofStrip, UrgencyBadge, MetricCard, PricingCard });
